...
function AuthScreen({clientId,onToken}){
  const[loading,setLoading]=useState(true); // start in loading — attempt silent first
  const[error,setError]=useState(null);

  // Attempt silent re-auth on mount — no UI shown if it succeeds
  useEffect(()=>{
    if(!window.google?.accounts?.oauth2){setLoading(false);return;}
    try{
  ...
  if(loading) return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:C.bg,color:C.green,gap:12}}>
      <div style={{fontSize:"2.5rem"}}>⚡</div>
      <div style={{fontWeight:700,fontSize:"1.1rem"}}>FND Tracker</div>
      <div style={{color:C.muted,fontSize:13}}>Signing in…</div>
    </div>
  );
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"100vh",background:C.bg,color:C.text,padding:"32px 24px",textAlign:"center"}}>
      ...
      <button onClick={signIn} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,padding:"14px 24px",borderRadius:12,border:`1.5px solid ${C.border}`,background:C.card,color:C.text,fontFamily:"inherit",fontSize:15,fontWeight:600,cursor:"pointer",minWidth:260,minHeight:52}}>
